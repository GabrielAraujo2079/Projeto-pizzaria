...
import { DatabaseConfig } from './types/InterfaceCent';
import { UsuarioRepository, ProdutoRepository, PedidoRepository, PromocaoRepository } from './repositories/CentralizadorRep';
import { PedidoService, PromocaoService, RelatorioService, NotaFiscalService, NotaFiscal, RelatorioVendas } from './services/services';
import * as bcrypt from 'bcrypt';
import { Usuario, Produto, Pedido, Promocao } from './models/Entidades';
...
export class PizzariaApp {
    ...
    async listarPedidosUsuario(usuarioId: number): Promise<Pedido[]> {
        // PedidoRepository pode fornecer findByUsuario; se não fornecer, fallback para filtrar por user_id
        if ((this.pedidoRepo as any).findByUsuario) {
            return await (this.pedidoRepo as any).findByUsuario(usuarioId);
        }

        const all = await this.pedidoRepo.findAll();
        return all.filter(p => p.clienteId === usuarioId || p.clienteId === undefined && p.clienteEmail && false);
    }
    ...
    async removerPedido(id: number): Promise<boolean> {
        return await (this.pedidoRepo as any).delete(id);
    }
