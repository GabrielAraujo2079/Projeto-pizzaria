...
import { PedidoRepository, PromocaoRepository, ProdutoRepository } from './repositores';
    ...
    async aplicarPromocoes(pedido: Pedido): Promise<void> {
        for (const item of pedido.itens) {
            const { desconto, promocao } = await this.promocaoService.aplicarPromocaoAoItem(
                { id: item.produtoId, nome: item.nomeProduto } as Produto,
                item.quantidade
            );
...
export class PromocaoService implements IPromocaoService {
    constructor(
