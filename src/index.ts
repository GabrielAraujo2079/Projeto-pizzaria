...
/*
  SISTEMA DE PIZZARIA - TypeScript
  - CRUD Admin: Clientes, Produtos, Pedidos, Promoções
  - Sistema de promoções diárias
  - Emissão de nota fiscal em TXT (data/notas-fiscais)
  - Relatório de vendas (data/Relatorio <mês>/relatorio_vendas.txt)
  Comentários explicativos em cada parte para facilitar manutenção.
*/

/* ===========================
   INTERFACES (modelos de dados)
   - Definem a forma esperada dos objetos
   =========================== */
...
interface Promocao {
    ...
    valorDesconto: number; // Percentual (0-100) ou valor em reais
    diaSemana: 0 | 1 | 2 | 3 | 4 | 5 | 6; // 0=domingo, 1=segunda, etc.
    categoriaAplicavel?: "pizza" | "bebida" | "sobremesa" | "todos";
    produtoEspecifico?: number; // ID do produto específico
...
interface Usuario {
    nome: string;
    senha: string; // já armazenada como hash
    cpf: string;
    email: string;
    telefone: string;
    endereco: {
...
interface ItemPedido {
    ...
    precoOriginal: number; // Preço antes do desconto
    descontoAplicado: number; // Valor do desconto
    subtotal: number;
    promocaoAplicada?: string; // Nome da promoção aplicada
}
...
interface Pedido {
    ...
    itens: ItemPedido[];
    subtotalOriginal: number; // Total antes dos descontos
    totalDescontos: number;   // Total de descontos aplicados
    ...
    observacoes?: string;
    promocoesAplicadas: string[]; // Nomes das promoções aplicadas
}
...
if (!fs.existsSync(arquivoUsuarios)) fs.writeFileSync(arquivoUsuarios, "[]");
if (!fs.existsSync(arquivoProdutos)) fs.writeFileSync(arquivoProdutos, "[]");
if (!fs.existsSync(arquivoPedidos)) fs.writeFileSync(arquivoPedidos, "[]");
if (!fs.existsSync(arquivoPromocoes)) fs.writeFileSync(arquivoPromocoes, "[]");

/* ===========================
   CARREGA DADOS NA MEMÓRIA
   =========================== */
let usuarios: Usuario[] = JSON.parse(fs.readFileSync(arquivoUsuarios, "utf-8"));
let produtos: Produto[] = JSON.parse(fs.readFileSync(arquivoProdutos, "utf-8"));
let pedidos: Pedido[] = JSON.parse(fs.readFileSync(arquivoPedidos, "utf-8"));
let promocoes: Promocao[] = JSON.parse(fs.readFileSync(arquivoPromocoes, "utf-8"));
...
function inicializarPromocoesDefault(): void {
...
function exibirPromocoesDoDia(): void {
...
function calcularDesconto(produto: Produto, quantidade: number, promocao: Promocao, precoUnitario: number): number {
...
function adicionarPromocao(): void {
    ...
    const tipoDesconto: "percentual" | "valor_fixo" = tipoInput === "1" ? "percentual" : "valor_fixo";
    ...
    let categoriaAplicavel: "pizza" | "bebida" | "sobremesa" | "todos";
    switch (categoriaInput) {
        case "1": categoriaAplicavel = "pizza"; break;
        case "2": categoriaAplicavel = "bebida"; break;
        case "3": categoriaAplicavel = "sobremesa"; break;
        case "4": categoriaAplicavel = "todos"; break;
        default: console.log("Categoria inválida!"); return;
    }
...
function menuPromocoes(): void {
    while (true) {
        ...
        switch (opcao) {
            case "1": adicionarPromocao(); break;
            case "2": listarPromocoes(); break;
            case "3": atualizarPromocao(); break;
            case "4": removerPromocao(); break;
            case "5": exibirPromocoesDoDia(); break;
            case "6": return;
...
function criarPedido(clienteEmail: string, clienteNome: string, clienteTelefone: string, clienteEndereco: Usuario['endereco']): void {
    console.log("\n=== FAZER PEDIDO ===");
    ...
    categorias.forEach(cat => {
        let titulo = "";
        if (cat === "pizza") titulo = "🍕 PIZZAS";
        if (cat === "bebida") titulo = "🥤 BEBIDAS";
        if (cat === "sobremesa") titulo = "🍨 SOBREMESAS";
        console.log("\n--- " + titulo + " ---");
    ...
    const precosTamanhos: { [key: string]: number } = {
        "Pequena": 25, "Media": 35, "Grande": 45, "Familia": 55
    };

    const itens: ItemPedido[] = [];
    const promocoesAplicadas: Set<string> = new Set();
    ...
    let tipoEntrega: "entrega" | "retirada";
    let enderecoEntrega: Pedido['enderecoEntrega'];
    ...
    switch (pagamentoInput) {
        ...
        case "3": formaPagamento = "debito"; break;
        case "4": formaPagamento = "credito"; break;
        default: formaPagamento = "dinheiro"; break;
    }
...
function emitirNotaFiscalTXT(pedido: Pedido): void {
...
function emitirNotaFiscal(pedido: Pedido): void {
    ...
    if (pedido.observacoes) console.log(`📝 Observações: ${pedido.observacoes}`);
    console.log("─".repeat(50));
...
function gerarRelatorioVendas(): void {
    ...
    if (!fs.existsSync(relatorioDir)) fs.mkdirSync(relatorioDir, { recursive: true });

    const relatorioPath = path.join(relatorioDir, "relatorio_vendas.txt");

    const vendasPorDia: { [data: string]: { qtd: number, valor: number, descontos: number } } = {};
    const vendasPorMes: { [mes: string]: { qtd: number, valor: number, descontos: number } } = {};
    const promocoesUsadas: { [nome: string]: { vezes: number, economia: number } } = {};
    ...
    pedidos.forEach(pedido => {
        ...
        if (!vendasPorDia[dia]) vendasPorDia[dia] = { qtd: 0, valor: 0, descontos: 0 };
        vendasPorDia[dia].qtd += qtd;
        vendasPorDia[dia].valor += valor;
        vendasPorDia[dia].descontos += descontos;

        if (!vendasPorMes[mes]) vendasPorMes[mes] = { qtd: 0, valor: 0, descontos: 0 };
        ...
        pedido.itens.forEach(item => {
            if (item.promocaoAplicada) {
                item.promocaoAplicada.split(",").map(p => p.trim()).forEach(nome => {
                    if (!promocoesUsadas[nome]) promocoesUsadas[nome] = { vezes: 0, economia: 0 };
...
function adicionarProduto(): void {
    ...
    let categoria: "pizza" | "bebida" | "sobremesa";
    switch (categoriaInput) {
...
function atualizarProduto(): void {
    ...
    const opcao = input("Escolha uma opção: ");
    switch (opcao) {
        case "1": produto.nome = input("Novo nome: ") || produto.nome; break;
        case "2": produto.descricao = input("Nova descrição: ") || produto.descricao; break;
        case "3":
        ...
        case "4":
            ...
            const categorias = { "1": "pizza" as const, "2": "bebida" as const, "3": "sobremesa" as const };
            if (catInput in categorias) produto.categoria = categorias[catInput as keyof typeof categorias];
            break;
        case "5":
        ...
        case "6":
            ...
            if (!isNaN(precoNovo) && precoNovo > 0) produto.preco = precoNovo;
            const novaCat = input("Nova categoria: ");
            const cats = { "1": "pizza" as const, "2": "bebida" as const, "3": "sobremesa" as const };
            if (novaCat in cats) produto.categoria = cats[novaCat as keyof typeof cats];
...
function listarTodosPedidos(): void {
    console.log("\n=== TODOS OS PEDIDOS ===");
    if (pedidos.length === 0) { 
    ...
    pedidos.forEach(p => {
        ...
        if (p.dataHoraStatus) console.log(`   Última alteração: ${new Date(p.dataHoraStatus).toLocaleString('pt-BR')}`);
...
function atualizarStatusPedido(): void {
    ...
    const statusMap: { [key: string]: Pedido['status'] } = { 
        "1": "pendente", 
        "2": "preparando", 
        "3": "pronto", 
        "4": "entregue", 
...
function menuCliente(usuario: Usuario): void {
    while (true) {
        console.log("\n=== MENU CLIENTE ===");
        ...
        const opcao = input("Escolha uma opção: ");
        switch (opcao) {
            case "1": exibirPromocoesDoDia(); break;
            case "2": criarPedido(usuario.email, usuario.nome, usuario.telefone, usuario.endereco); break;
            case "3": listarMeusPedidos(usuario.email); break;
            case "4": mostrarMeuEndereco(usuario); break;
            case "5": return;
            default: console.log("Opção inválida!"); break;
        }
    }
...
function mostrarMeuEndereco(usuario: Usuario): void {
    console.log("\n=== MEU ENDEREÇO ===");
...
function listarMeusPedidos(clienteEmail: string): void {
    console.log("\n=== MEUS PEDIDOS ===");
...
while (true) {
    ...
    switch (opcao) {
        ...
        case "2": login(); break;
        case "3": exibirPromocoesDoDia(); break;
        case "4": console.log("Saindo do sistema..."); process.exit(0);
        default: console.log("Opção inválida!"); break;
    }
}